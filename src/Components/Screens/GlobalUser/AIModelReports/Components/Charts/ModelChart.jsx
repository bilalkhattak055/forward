...
// Ensure series2 is always an array, even if the API returns an empty object
...
const formattedSeriesData = [
    {
        name: '80 > value',
        data: safeSeries2.map(item =>
            item && Object.keys(item).length > 0 && item?.value < 80 ? item?.value : 0
        ),
        color: '#ef4343'
    },
    {
        name: '95> value > 80',
        data: safeSeries2.map(item =>
            item && Object.keys(item).length > 0 && item?.value >= 80 && item?.value <= 95 ? item?.value : 0
        ),
        color: '#f9c50a'
    },
    {
        name: '95 < value',
        data: safeSeries2.map(item =>
            item && Object.keys(item).length > 0 && item?.value > 95 ? item?.value : 0
        ),
        color: '#32CD32'
    }
];
    ...
    const secondOptions = {
        ...
        xaxis: {
            categories: safeSeries2?.map(item =>   nameAbbreviations[item?.name] || item?.name), // Extract the names for the x-axis categories
            labels: {
                style: {
    ...
    return (
        <>
            <ReactApexChart
                ...
                series={formattedSeriesData ? formattedSeriesData : null} // Updated series with new data format
                type="bar"
                height={350}
