...
const TargetsAndControls = ({ area: areaD }) => {
  ...
  const handleweekChange = (e) => {
    ...
    if (value?.trim() !== "") {
      ...
      setFilters({
        ...filters,
        [name]: moment().format("YYYY") + "-W" + moment().format("WW"),
      }); 
      setTargetFiltersContext({
        ...filters,
        [name]: moment().format("YYYY") + "-W" + moment().format("WW"),
                          ...
                          <CardBody className="mt-0 pt-0">
                            ...
                            <div className="d-flex align-items-center justify-content-between">
                              ...
                              {maxWeek?.split('-')[1]?.slice(1, 3) == a.current_week.week ?
                                <div style={{width:'max-content', cursor:'pointer'}} onClick={() => handleUpdate(a)} className="d-flex justify-content-center align-items-center"><span style={{fontSize:'13px', cursor:'pointer'}} className="f-light"> {percDed === 0 || !percDed ? "Set Target" : " Update Target"}</span> <AiOutlineEdit type="button"  style={{ fontSize: '15px', color: '#7e7a7e', fontWeight: '800', cursor: 'pointer' }} /></div>
                                : null}
                            </div>
